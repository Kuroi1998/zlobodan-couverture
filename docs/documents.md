...
(`src/__tests__/documentAccess.test.ts`).
...
| Fichier | Portée |
| --- | --- |
| `src/__tests__/pdfEngine.test.ts` | Translittération, découpe, pagination, métadonnées |
| `src/__tests__/documentTemplate.test.ts` | Contenu réellement imprimé, décodé depuis les flux |
| `src/__tests__/documentAccess.test.ts` | Matrice d'autorisation, noms, clés, en-têtes |
