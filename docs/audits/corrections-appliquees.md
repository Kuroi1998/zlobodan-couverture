...
| **F2** | **Corrigé** | Vitest installé. [accessControl.test.ts](../../test/unit/accessControl.test.ts) importe le vrai `can()` au lieu de le réimplémenter. `businessLogic.test.ts` importe ses dépendances. `npm test` exécute réellement 50 tests. |
