    ...
    <footer className="bg-slate-950 text-slate-300 pt-16 pb-24 md:pb-12 border-t border-slate-800">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 pb-12 border-b border-slate-800">
          ...
          <div className="space-y-4">
            ...
            <div className="pt-2 space-y-2 text-xs text-slate-400">
              <p className="flex items-center gap-2">
                ...
                <span>N° BCE / TVA : {siteData.siret}</span>
              </p>
              <p className="flex items-center gap-2">
          ...
          <div className="space-y-4">
            <h3 className="font-heading font-bold text-lg text-white tracking-wide border-b border-slate-800 pb-2">
              Zone d'Intervention ({siteData.radiusKm}km)
            </h3>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3 text-sm">
              ...
              <p className="flex items-center gap-2.5">
                ...
                <a href={`tel:${siteData.phone}`} className="hover:underline font-bold text-white">
                  {siteData.phoneFormatted}
                </a>
              </p>
              <p className="flex items-center gap-2.5">
                <Mail className="h-5 w-5 text-amber-400 shrink-0" />
                <a href={`mailto:${siteData.email}`} className="hover:underline text-slate-300 text-xs">
                  {siteData.email}
                </a>
              </p>
              <div className="pt-2 border-t border-slate-900 text-xs space-y-1">
                <p className="flex items-center gap-1.5 text-slate-400">
                  ...
                  <span>{siteData.openingHours.days}</span>
                </p>
                <p className="text-white font-medium pl-5">{siteData.openingHours.hours}</p>
        ...
        <div className="pt-8 flex flex-col md:flex-row items-center justify-between text-xs text-slate-500 gap-4">
          <p>© {new Date().getFullYear()} {siteData.name}. Tous droits réservés. SRL au capital de {siteData.capital}.</p>
