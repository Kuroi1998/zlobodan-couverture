# Politique de sécurité — Zlobodan Couverture

**Dernière révision** : 28 juillet 2026
**Portée** : application web Next.js (`src/`), sa configuration, ses dépendances.
**Audit de référence** : [docs/audits/audit-securite-2026-07-25.md](docs/audits/audit-securite-2026-07-25.md)
...
## 0. Versions maintenues

Le projet n'a pas encore fait l'objet d'une publication versionnée. Seule la
branche `main` est maintenue et reçoit les correctifs de sécurité.

| Version | Maintenue |
| --- | --- |
| `main` (branche courante) | Oui |
| Tout tag ou branche d'archive antérieur | Non |

Les branches `archive/*` et les tags `legacy-*` conservent l'historique à des
fins de traçabilité. Ils ne reçoivent aucun correctif et ne doivent pas être
déployés.
...
**Ne jamais signaler une vulnérabilité dans une issue GitHub, une pull request,
un commit ou tout autre espace public.** Un signalement public expose les
utilisateurs avant qu'un correctif n'existe.

Canaux privés, par ordre de préférence :

1. **GitHub Security Advisories** — onglet *Security* du dépôt, « Report a
   vulnerability ». Canal privé de bout en bout, à privilégier.
2. **Email** : `security@zlobodan-couverture.be`
3. **Point de terminaison** : `/.well-known/security.txt` (RFC 9116)

### Informations à transmettre

Pour permettre une reproduction rapide, indiquer :

- le type de vulnérabilité et le composant concerné (route, service, fichier) ;
- les étapes de reproduction, aussi précises que possible ;
- une preuve de concept minimale, si elle existe ;
- l'impact estimé (lecture de données, élévation de privilège, déni de
  service…) ;
- la version ou le commit testé, et l'environnement utilisé ;
- vos coordonnées pour le suivi.

### Délais

- **Accusé de réception** : sous 72 heures ouvrées.
- **Première évaluation** : sous 7 jours ouvrés, avec une qualification de
  gravité.
- **Correction** : aucun délai n'est garanti. Il dépend de la gravité et de la
  complexité. L'avancement est communiqué au rapporteur.

Ce projet est maintenu par une équipe réduite : ces délais sont un engagement
de moyens, pas de résultat.

### Règles d'engagement

Merci de ne pas exécuter de test de déni de service, de ne pas accéder à des
données clientes réelles, de ne pas altérer ni exfiltrer de données, et de nous
laisser un délai raisonnable avant toute publication.

Aucun programme de récompense (*bug bounty*) n'est en place.
